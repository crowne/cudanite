...
public class CudaDevice {
    ...
    /**
     * Construct a new CudaDevice wrapper object.
     * 
     * @param device
     * @throws NullPointerException
     *             if device is null
     */
    public CudaDevice(final CUdevice device) {
        if (device == null) {
    ...
    public String getName() {
        ...
        int result = JCudaDriver.cuDeviceGetName(deviceName, deviceName.length, device);
        if (result == CUresult.CUDA_SUCCESS) {
            name = StringUtil.createString(deviceName);
        }
     ...
     * @return
     */
    public CudaComputeCapability getComputeCapatility() {
        int result;
        int resultArray[] = { 0 };
        int major = 0;
        int minor = 0;

        result = JCudaDriver.cuDeviceGetAttribute(resultArray,
                CUdevice_attribute.CU_DEVICE_ATTRIBUTE_COMPUTE_CAPABILITY_MAJOR, device);

        if (result == CUresult.CUDA_SUCCESS) {
            major = resultArray[0];
            result = JCudaDriver.cuDeviceGetAttribute(resultArray,
                    CUdevice_attribute.CU_DEVICE_ATTRIBUTE_COMPUTE_CAPABILITY_MINOR, device);
            if (result == CUresult.CUDA_SUCCESS) {
                minor = resultArray[0];
            }
        }

        CudaComputeCapability ccm = new CudaComputeCapability(major, minor);

        return ccm;
    }

    /**
     * Prints all the attribute values for the device to sysout
     */
     ...
     * 
     * @param attribute
     *            The CUdevice_attribute constant
